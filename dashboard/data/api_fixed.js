/**
 * Newsletter Dashboard Data API - Fixed Version
 * Aggregates data from newsletter JSON files with proper article extraction
 */
...
function loadData() {
    ...
    for (const file of files) {
        ...
        // Process articles from story_rollup
        if (newsletter.story_rollup) {
            for (const story of newsletter.story_rollup) {
                if (story.headline && story.source) {
                    // Try to find URL from knowledge graph nodes
                    let url = '#';
                    if (newsletter.knowledge_graph?.nodes) {
                        const articleNode = newsletter.knowledge_graph.nodes.find(n => 
                            n.kind === 'article' && n.title && n.title.includes(story.headline.substring(0, 50))
                        );
                        if (articleNode?.url) {
                            url = articleNode.url;
                        }
                    ...
                    allArticles.push({
                        ...
                        source: story.source,
                        url: url,
                        summary: story.summary || story.what_happened || '',
                        date: newsletter.generated_at || new Date().toISOString(),
                        labels: [], // We'll need to extract labels from edges
                        theme: story.theme_bucket || 'General'
                    });
                }
            }
        }
